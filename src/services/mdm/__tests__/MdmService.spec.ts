	...
	t: vi.fn((key: string) => {
		const translations: Record<string, string> = {
			"mdm.errors.cloud_auth_required":
				"Your organization requires CodiDroid Cloud authentication. Please sign in to continue.",
			"mdm.errors.organization_mismatch":
				"You must be authenticated with your organization's CodiDroid Cloud account.",
	...
	describe("compliance checking", () => {
		...
		it("should be non-compliant when not authenticated", async () => {
			...
			if (!compliance.compliant) {
				expect(compliance.reason).toContain("Your organization requires CodiDroid Cloud authentication")
			}
		})
