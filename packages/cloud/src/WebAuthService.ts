...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			this.log(`[auth] Error initiating CodiDroid Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate CodiDroid Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from CodiDroid Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with CodiDroid Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid CodiDroid Cloud sign in url")
			}
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with CodiDroid Cloud")
			}

			this.log("[auth] Successfully authenticated with CodiDroid Cloud")
		} catch (error) {
			this.log(`[auth] Error handling CodiDroid Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle CodiDroid Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from CodiDroid Cloud")
			}

			this.log("[auth] Logged out from CodiDroid Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from CodiDroid Cloud: ${error}`)
			throw new Error(`Failed to log out from CodiDroid Cloud: ${error}`)
		}
	}
